const C='fluo-saeiv-v11-annonces-arret';
...
self.addEventListener('fetch',e=>{
  ...
  // Le fond OpenStreetMap/Leaflet reste réseau : on évite de remplir le stockage iPhone de tuiles cartographiques.
  ...
  e.respondWith(caches.match(e.request).then(hit=>hit||fetch(e.request).then(r=>{const cp=r.clone();caches.open(C).then(c=>c.put(e.request,cp)).catch(()=>{});return r;})));
});
