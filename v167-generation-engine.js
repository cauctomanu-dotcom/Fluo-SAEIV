'use strict';
/* Mon SAEIV 1.0.69 — génération exploitation tenant compte de la prise/fin de service.
   Les éléments automatiques HLP/coupures/prise-fin de service sont reconstruits côté serveur
   après chaque proposition, puis la RSE valide ou refuse uniquement le segment testé. */
(()=>{
  ...
  function asActivity(x){return{id:x.payload?.segment_id||x.id,line:x.line||'',type:x.type,start:String(x.start_time||'').slice(0,5),end:String(x.end_time||'').slice(0,5),origin:x.origin||'',destination:x.destination||'',originCoords:x.origin_coords||null,destinationCoords:x.destination_coords||null,driveMinutes:Number(x.drive_minutes)||duration(x.start_time,x.end_time),regime:x.regime||'national50'}}
  ...
  function scoreCandidate(seg,driver,current,parking){
    ...
    let prev=null,next=null;for(const x of xs){const a=mm(x.start),b=mm(x.end);if(a===null||b===null)continue;let bb=b;if(bb<a)bb+=1440;if(bb<=s)prev=x;else if(a>=e&&!next)next=x}
    ...
    if(fm!==null&&le0!==null){let le=le0;if(le<fm)le+=1440;const firstHlp=estimateMinutes(hav(park,first.originCoords)||0),lastHlp=estimateMinutes(hav(last.destinationCoords,park)||0);amplitude=(le+lastHlp+5)-(fm-firstHlp-10)}
    if(amplitude>780)return{ok:false,reason:'amplitude estimée > 13 h (prise/fin de service incluses)'};
    const work=all.reduce((z,x)=>z+(['cut','pause'].includes(x.type)?0:duration(x.start,x.end)),0)+m1+m2+15;if(work>600)return{ok:false,reason:'travail estimé > 10 h (prise/fin de service incluses)'};
  ...
  async function fetchState(date){const c=client(),p=profile();const [{data:drivers,error:de},{data:items,error:ie},{data:settings,error:se}]=await Promise.all([c.from('profiles').select('user_id,matricule,display_name,active').eq('organization_id',p.organization_id).eq('role','driver').eq('active',true),c.from('plan_items').select('*').eq('organization_id',p.organization_id).eq('service_date',date).order('start_time',{ascending:true,nullsFirst:false}),c.from('driver_settings').select('user_id,bus_parking').eq('organization_id',p.organization_id)]);if(de)throw de;if(ie)throw ie;if(se)throw se;const parking=new Map();for(const s of settings||[])if(validPoint(s.bus_parking))parking.set(String(s.user_id),s.bus_parking);return{drivers:drivers||[],items:items||[],parking}}
  ...
  async function generate(){
    ...
    running=true;const button=q('v165Generate');if(button){button.disabled=true;button.textContent='✨ GÉNÉRATION EN COURS…'}const date=dateValue();let placed=0,rejectedByServer=0,examined=0,lastReject='';
    try{await b.refresh?.();let{drivers,items,parking}=await fetchState(date);if(!drivers.length)throw new Error('Aucun conducteur actif dans la société.');const usable=drivers.filter(d=>parking.has(String(d.user_id)));if(!usable.length)throw new Error('Aucun conducteur n’a de « Stationnement bus » géolocalisé.');const{data:{user}}=await c.auth.getUser();const working=new Map(drivers.map(d=>[String(d.user_id),workingItems(items,d.user_id)]));const free=allSegments.filter(s=>!assignmentFor(s,items)).slice().sort((a,b)=>(mm(a.start)??9999)-(mm(b.start)??9999)||String(a.line||'').localeCompare(String(b.line||''),'fr',{numeric:true}));if(!free.length){status('Toutes les courses sont déjà placées.','ok');return}status(`Génération sûre · ${free.length} segments à étudier…`,'busy');
      for(const seg of free){examined++;const ranked=[];for(const d of usable){const r=scoreCandidate(seg,d,working.get(String(d.user_id))||[],parking);if(r.ok)ranked.push({d,r})}ranked.sort((a,b)=>a.r.score-b.r.score);if(!ranked.length)continue;
        for(const cand of ranked){const driverId=String(cand.d.user_id),cid=clientId(seg);status(`✨ ${placed} placé${placed>1?'s':''} · test ${seg.line||'course'} ${seg.start} → ${cand.d.display_name||cand.d.matricule} · ${examined}/${free.length}`,'busy');const{error:ins}=await c.from('plan_items').upsert(rowFor(seg,driverId,user?.id||null,p.organization_id,date),{onConflict:'driver_user_id,client_id'});if(ins){lastReject=ins.message||String(ins);rejectedByServer++;continue}let check=null,reason=null;try{check=await invokeRebuild(driverId,date);reason=rejectReason(check)}catch(err){reason=err?.message||String(err)}if(reason){lastReject=reason;rejectedByServer++;await c.from('plan_items').delete().eq('driver_user_id',driverId).eq('client_id',cid);try{await invokeRebuild(driverId,date)}catch{}continue}placed++;working.get(driverId).push(seg);working.get(driverId).sort((a,b)=>(mm(a.start)??9999)-(mm(b.start)??9999));items.push(rowFor(seg,driverId,user?.id||null,p.organization_id,date));break}
        if(examined%80===0)await new Promise(r=>setTimeout(r,0));
      }
      await b.refresh?.();const remaining=allSegments.filter(s=>!assignmentFor(s,b.items||[])).length;if(placed)status(`✅ Génération terminée : ${placed} segment${placed>1?'s':''} placé${placed>1?'s':''}. Prise de service + HLP + coupures + fin de service recalculés avec contrôle RSE · ${remaining} non placé${remaining>1?'s':''}.`,'ok');else status(`⚠ 0 segment placé. ${rejectedByServer?`${rejectedByServer} proposition${rejectedByServer>1?'s':''} refusée${rejectedByServer>1?'s':''} par HLP/RSE. `:''}${lastReject?`Dernier motif : ${lastReject}`:'Aucun enchaînement compatible avec les conducteurs disponibles.'}`,'err');
  ...
  window.MonSAEIVGenerationEngineV167={installed:true,version:VERSION,generate,get running(){return running}};
})();