...
(()=>{
  ...
  function gapEconomy(a,b,park){
    ...
    const directKm=hav(a.destinationCoords,b.originCoords);if(directKm===null)return{ok:false,reason:'coordonnées HLP intermédiaire manquantes'};const directMin=estimateMinutes(directKm);if(gap<directMin)return{ok:false,reason:`HLP impossible entre ${a.line||'course'} et ${b.line||'course'}`};
    const directCut=gap-directMin,directPct=near(b.originCoords,park)?0:50,directCost=fuelCost(directKm*1.18)+paidCutCost(directCut,directPct);
    ...
    const k1=hav(a.destinationCoords,park),k2=hav(park,b.originCoords);if(k1!==null&&k2!==null){const m1=estimateMinutes(k1),m2=estimateMinutes(k2);if(m1+m2<=gap){const cut=Math.max(0,gap-m1-m2),viaKm=(k1+k2)*1.18,viaCost=fuelCost(viaKm)+((m1+m2)/60)*ECON.referenceLaborEurPerHour;if(viaCost+0.01<chosen.costEur)chosen={mode:'parking',hlpKm:viaKm,hlpMinutes:m1+m2,cutMinutes:cut,cutPct:0,costEur:viaCost,paidCutMinutes:0}}}
    return{ok:true,...chosen};
  }

  function scheduleEconomy(all,park){
    const xs=[...all].sort((a,b)=>(mm(a.start)??9999)-(mm(b.start)??9999));if(!xs.length)return{ok:true,hlpKm:0,hlpMinutes:0,cutMinutes:0,paidCutMinutes:0,cutCount:0,maxCut:0,maxPaidCut:0,costEur:0,amplitude:0,work:0,drive:0,score:0};
    const first=xs[0],last=xs.at(-1),firstKm=hav(park,first.originCoords),lastKm=hav(last.destinationCoords,park);if(firstKm===null||lastKm===null)return{ok:false,reason:'coordonnées HLP début/fin manquantes'};
    let hlpKm=(firstKm+lastKm)*1.18,hlpMinutes=estimateMinutes(firstKm)+estimateMinutes(lastKm),cutMinutes=0,paidCutMinutes=0,costEur=fuelCost(hlpKm)+(hlpMinutes/60)*ECON.referenceLaborEurPerHour;const cuts=[],paidCuts=[];
    for(let i=0;i<xs.length-1;i++){const g=gapEconomy(xs[i],xs[i+1],park);if(!g.ok)return g;hlpKm+=g.hlpKm;hlpMinutes+=g.hlpMinutes;cutMinutes+=g.cutMinutes;paidCutMinutes+=g.paidCutMinutes;costEur+=g.costEur;if(g.cutMinutes>0){cuts.push(g.cutMinutes);if(g.cutPct===50)paidCuts.push(g.cutMinutes)}}
    ...
    const courseWork=xs.reduce((z,x)=>z+duration(x.start,x.end),0),work=courseWork+hlpMinutes+15,drive=xs.reduce((z,x)=>z+(Number(x.driveMinutes)||duration(x.start,x.end)),0)+hlpMinutes,maxCut=cuts.length?Math.max(...cuts):0,maxPaidCut=paidCuts.length?Math.max(...paidCuts):0,cutCount=cuts.length;
    let score=costEur*100+cutCount*170+paidCutMinutes*3+amplitude*.05;if(cutCount>2)score+=(cutCount-2)*350;if(maxPaidCut>=120)score+=2500+(maxPaidCut-120)*15;if(maxCut>=240)score+=(maxCut-240)*4;
