...
(()=>{
  ...
  function setEntryMode(mode){try{localStorage.setItem(ENTRY_MODE_KEY,mode)}catch{}}
  function setCloudStatus(text,kind=''){
    const el=q('v156CloudStatus');if(el){el.textContent=text;el.className=`v156-cloud-status ${kind}`}
    const chip=q('v156CloudChip');if(chip){chip.textContent=S.profile?`☁ ${S.profile.matricule} · ${roleLabel(S.profile.role)}`:'☁ Synchronisation serveur'}
  }

  function prefillFromLocal(){
    const a=localAccount();if(!a?.matricule)return;
    if(q('v156RegMatricule')&&!q('v156RegMatricule').value)q('v156RegMatricule').value=a.matricule;
    const net=a.networkKey||a.network||a.network_key;if(net&&q('v156RegNetwork')?.querySelector(`option[value="${CSS.escape(String(net))}"]`))q('v156RegNetwork').value=net;
    const info=q('v156LegacyInfo');if(info)info.innerHTML=`Profil conducteur local détecté : <b>matricule ${esc(a.matricule)}</b>. Il reste conservé sur cet appareil.`;
  }
  function ensureLocalCloudEntry(){
    const bar=q('v13Userbar');if(!bar||q('v156CloudChip')||q('v156LocalCloudEntry'))return;
    const b=document.createElement('button');b.id='v156LocalCloudEntry';b.type='button';b.textContent='☁ Activer la synchronisation';b.addEventListener('click',showCloudLogin);bar.appendChild(b);
  }
  function showLocalAccount(persist=true){
    const a=localAccount();if(!a)return showCloudLogin();if(persist)setEntryMode('local');
    q('v156CloudAuth')?.classList.add('hidden');q('v157Dispatch')?.classList.add('hidden');
    const auth=q('v13Auth');if(auth)auth.classList.remove('hidden');
    let tries=0;const ensure=()=>{ensureLocalCloudEntry();if(!q('v156LocalCloudEntry')&&++tries<50)setTimeout(ensure,100)};ensure();
  }
  function showCloudLogin(){
    setEntryMode('cloud');q('v13Auth')?.classList.add('hidden');q('v156CloudAuth')?.classList.remove('hidden');prefillFromLocal();setCloudStatus(localAccount()?'Ton profil conducteur local est conservé. Tu peux le lier au serveur sans le remplacer.':'Connecte-toi pour activer la synchronisation.');
  }
  ...
  function addUi(){
    ...
    const style=document.createElement('style');style.id='v156CloudStyle';style.textContent=`
      ...
      .v156-legacy{margin:12px 0;padding:11px;border:1px solid #3b7250;border-radius:13px;background:#0c2b1a}.v156-legacy b{color:#b9ffd0}.v156-legacy p{margin:0 0 8px;color:#c5dfce}.v156-legacy-actions{display:grid;grid-template-columns:1fr 1fr;gap:7px}.v156-legacy-actions button:first-child{border-color:#4d9b68;background:#153e25}
      #v156CloudChip,#v156LocalCloudEntry{min-height:36px!important;padding:6px 9px!important;font-size:.66rem!important}
      ...
      @media(max-width:600px){.v156-cloud-grid,.v156-account-actions,.v156-legacy-actions{grid-template-columns:1fr}.v156-cloud-grid .wide{grid-column:auto}.v156-cloud-auth{padding:0;align-items:flex-end}.v156-cloud-card{border-radius:22px 22px 0 0;max-height:97dvh}}
    ...
    document.body.insertAdjacentHTML('beforeend',`
      <div id="v156CloudAuth" class="v156-cloud-auth">
        <div class="v156-cloud-card">
          ...
          <p>Le compte serveur ajoute la synchronisation multi-appareils et les plannings transmis par l’exploitation. Il ne remplace pas ton profil conducteur local existant.</p>
          <div id="v156LegacyBox" class="v156-legacy hidden"><p id="v156LegacyInfo"></p><div class="v156-legacy-actions"><button id="v156UseLocal" type="button">👤 Ouvrir mon profil conducteur existant</button><button id="v156LinkCloud" type="button">☁ Lier ce profil au serveur</button></div></div>
          <div class="v156-cloud-tabs"><button id="v156LoginTab" class="active" type="button">Se connecter au serveur</button><button id="v156RegisterTab" type="button">Créer / lier un compte</button></div>
          ...
          <form id="v156RegisterForm" class="v156-cloud-form hidden">
            ...
            <button class="primary" type="submit">CRÉER / LIER LE COMPTE</button>
          </form>
    ...
    `);
    const local=localAccount(),legacy=q('v156LegacyBox');if(local?.matricule){legacy?.classList.remove('hidden');prefillFromLocal()}
    q('v156UseLocal')?.addEventListener('click',()=>showLocalAccount(true));q('v156LinkCloud')?.addEventListener('click',()=>{switchMode('register');prefillFromLocal();setEntryMode('cloud')});
  ...
  function switchMode(mode){const reg=mode==='register';q('v156LoginForm')?.classList.toggle('hidden',reg);q('v156RegisterForm')?.classList.toggle('hidden',!reg);q('v156LoginTab')?.classList.toggle('active',!reg);q('v156RegisterTab')?.classList.toggle('active',reg);if(reg)prefillFromLocal();setCloudStatus('')}
  ...
  async function register(e){
    e.preventDefault();setEntryMode('cloud');const email=q('v156RegEmail').value.trim(),password=q('v156RegPassword').value,confirm=q('v156RegConfirm').value;if(password!==confirm){setCloudStatus('Les deux mots de passe ne correspondent pas.','err');return}
  ...
  async function afterAuth(session){
    ...
    S.profile=profile;setEntryMode('cloud');showAccountButton();setCloudStatus(`Connecté · ${roleLabel(profile.role)}`,'ok');q('v156CloudAuth')?.classList.add('hidden');
  ...
  async function init(){
    ...
    try{const c=await loadClient();const {data:{session}}=await c.auth.getSession();if(session)await afterAuth(session);else if(localAccount()&&localStorage.getItem(ENTRY_MODE_KEY)!=='cloud'){showLocalAccount(false)}else{q('v156CloudAuth')?.classList.remove('hidden');prefillFromLocal();setCloudStatus(localAccount()?'Ton ancien profil conducteur est conservé. Tu peux l’ouvrir ou activer la synchronisation.':'Connecte-toi pour activer la synchronisation.')}
      c.auth.onAuthStateChange((event,session)=>{if(event==='SIGNED_OUT'){if(localAccount()&&localStorage.getItem(ENTRY_MODE_KEY)==='local')showLocalAccount(false);else{q('v156CloudAuth')?.classList.remove('hidden');prefillFromLocal()}}else if(session&&event==='SIGNED_IN'&&!S.user)afterAuth(session).catch(e=>setCloudStatus(e.message||e,'err'))});
    }catch(e){console.error('[Mon SAEIV] Supabase init',e);if(localAccount()){showLocalAccount(false)}else{setCloudStatus('Serveur indisponible : '+(e.message||e),'err');q('v156CloudAuth')?.classList.remove('hidden')}}
    finally{S.initializing=false}
  }

  window.MonSAEIVCloudV156={installed:true,version:VERSION,get client(){return S.client},get profile(){return S.profile},get user(){return S.user},sync:async()=>{await pushPlanning();await pullPlanning()},pullPlanning,pushPlanning,showCloudLogin,showLocalAccount,logout:logoutCloud};
