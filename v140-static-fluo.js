'use strict';
/* Mon SAEIV 1.0.40 — données Fluo 54/67/68 préparées côté GitHub Pages.
...
(()=>{
  ...
  function displayRoute(dept,route,date=selectedIso()){
    ...
    if(d==='54'&&String(date)>=CUTOVER){
      ...
      if(m)r.short=String(Number(m[1]));
    }else if(d==='68'){
  ...
  async function staticCore(dept){
    ...
    return {format:'fluo-static-gtfs-v140',department:d,routesIndex,servicesIndex,stopsIndex};
  }
  ...
  console.info('[Mon SAEIV] données Fluo statiques 1.0.40 actives');
})();
